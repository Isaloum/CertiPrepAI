...
| ✅ Done (Jul 18 2026) | CI now runs **lint (blocking)** + **vitest (blocking)** + build. Tests: `react-app/src/**/*.test.ts` (`npm test`); first suite `src/lib/tiers.test.ts` (14 tests). Lint is at **0 errors**: fixed all real ones (unused vars, empty catches, `any` types, ternary-as-statement, `Date.now()`-in-render purity via mount-captured `const [now]=useState(()=>Date.now())`, a stray `b2` data key + added a `Comparison` interface). The 3 React-Compiler/HMR advisory rules (`set-state-in-effect`, `preserve-manual-memoization`, `only-export-components`) are set to **warn** in `eslint.config.js` (19 warnings remain, non-blocking — they flag legitimate load-in-effect/reset-on-change patterns, not bugs). | `.github/workflows/ci.yml`, `eslint.config.js` | — |
