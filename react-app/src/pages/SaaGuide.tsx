...
const MATRIX: MatrixRow[] = [
  ...
  // ── NEW EXAM-TESTED SCENARIOS ────────────────────────────────────────────────
  { requirement: 'Lambda function getting "too many connections" errors to RDS', solution: 'RDS Proxy', why: 'Lambda can scale to tens of thousands of concurrent instances, each opening a new DB connection. RDS Proxy pools and reuses connections, aggregating thousands of Lambda connections into a much smaller pool to RDS. Eliminates the connection exhaustion error.' },
  { requirement: 'Clients need to whitelist a static IP at their firewall to reach your load balancer', solution: 'Network Load Balancer with Elastic IP', why: 'NLB supports one Elastic IP per AZ. ALB does NOT support Elastic IPs — its underlying IPs are dynamic and DNS-based. Use NLB when clients must whitelist a fixed IP. Optionally put an ALB behind NLB if Layer 7 path routing is also needed.' },
  { requirement: 'Video surveillance — detect unauthorised persons entering restricted areas + alert via SMS', solution: 'Kinesis Video Streams + Amazon Rekognition + SNS', why: 'Kinesis Video Streams ingests live camera feeds at scale. Rekognition Video analyses frames for face detection and object recognition without any ML training. SNS publishes SMS alerts to the security team on detection.' },
  { requirement: 'Route more traffic to one AWS region by expanding its geographic coverage', solution: 'Route 53 Geoproximity Routing with positive bias', why: 'Geoproximity lets you expand (+) or shrink (−) the geographic region that routes to a resource using a bias value. Unlike Geolocation, which routes by exact country/continent, Geoproximity lets you pull in traffic from adjacent regions.' },
  { requirement: 'VPC IPv4 CIDR exhausted — new EC2 instance cannot launch', solution: 'Create an IPv6-only subnet', why: 'You cannot remove IPv4 from an existing VPC — it is mandatory. You CAN add an IPv6-only subnet. New resources in it use IPv6 addresses, bypassing IPv4 exhaustion. The VPC continues to run in dual-stack mode.' },
  { requirement: 'Track and report AWS spending broken down by department or business unit', solution: 'Tag resources + enable Cost Allocation Tags in Billing', why: 'Tag each resource with the department name. Activate the tag as a Cost Allocation Tag in the Billing console. AWS generates a Cost Allocation Report (CSV) grouped by active tags. Cost Explorer Resource filter only covers EC2 — insufficient for all-service tracking.' },
  { requirement: 'Priority processing queue — premium users processed before free users', solution: 'Two separate SQS queues — consumers poll premium queue first', why: 'SQS has no per-message priority. Create one queue for premium and one for free. EC2/Lambda consumers always check and drain the premium queue first; they only poll the free queue when premium is empty. Simple, scalable, proven.' },
  { requirement: 'Access DynamoDB from within a VPC without traffic going over the public internet', solution: 'DynamoDB Gateway VPC Endpoint (free)', why: 'DynamoDB uses a Gateway endpoint — completely free, no PrivateLink charges. Associate the endpoint with your route table and traffic stays on the AWS private network. Do NOT create an Interface endpoint for DynamoDB — that costs money and is incorrect.' },
  { requirement: 'Automated cross-account or cross-region backup of DynamoDB table', solution: 'AWS Backup', why: 'DynamoDB on-demand backups CANNOT be copied to a different account or region natively. AWS Backup adds this capability. Enable DynamoDB advanced features by opting in to AWS Backup in the DynamoDB console, then configure cross-account backup plans.' },
  { requirement: 'AI content moderation for images uploaded to S3 — no ML expertise, private VPC access', solution: 'Amazon Rekognition + Interface VPC Endpoint', why: 'Rekognition Content Moderation detects explicit/inappropriate content out of the box (no training required). For private VPC access, create an Interface VPC Endpoint for Rekognition so traffic never leaves the AWS network.' },
  { requirement: 'On-premises tape backup — migrate to cloud, retain 10 years, accessed once or twice a year', solution: 'Storage Gateway Tape Gateway → S3 Glacier Deep Archive', why: 'Tape Gateway exposes a virtual tape library to existing backup software (no workflow changes). Tapes are archived to Glacier Deep Archive — the cheapest AWS storage at ~$0.00099/GB/month, ideal for decade-long retention with annual access.' },
  { requirement: 'Control which S3 buckets are accessible through a VPC Gateway Endpoint (least effort)', solution: 'Gateway Endpoint Policy restricting access to trusted S3 buckets', why: 'One endpoint policy covers all traffic through the endpoint and can whitelist specific bucket ARNs. Far more efficient than adding a bucket policy to each individual S3 bucket. Endpoint policy supplements but does not replace bucket policies.' },
  { requirement: 'Block traffic to known malicious URLs and blacklisted FQDNs at the VPC perimeter', solution: 'ALB in public subnet + AWS Network Firewall (reroute VPC traffic through firewall endpoints)', why: 'Network Firewall supports Suricata-based stateful rules including domain name allow/deny lists for URL and FQDN filtering. WAF (used with ALB) handles HTTP-level rules (SQL injection, rate limiting) — but domain/FQDN blocking requires Network Firewall at the VPC level.' },
  { requirement: 'Big data ETL processing (Hadoop/Spark) + BI analytics with standard SQL queries', solution: 'Amazon EMR (process/transform) → Amazon Redshift (analytics/BI)', why: 'EMR runs open-source big data frameworks to transform large datasets. Load processed data into Redshift for massively parallel SQL analytics compatible with BI tools (QuickSight, Tableau). EMR = compute and transform; Redshift = query and visualise.' },
  { requirement: 'Container orchestration that is open-source, cloud-agnostic, portable across providers', solution: 'Amazon EKS (Kubernetes)', why: 'Kubernetes is open-source and cloud-agnostic — the same manifests run on AWS, Azure, GCP, or on-premises. ECS is AWS-proprietary and cannot be migrated to another cloud. Any question mentioning "cloud-agnostic" or "same tooling across providers" → EKS, never ECS.' },
  { requirement: 'Monitor EC2 swap space, memory utilization, or per-process OS-level metrics', solution: 'Install CloudWatch Agent on each EC2 instance', why: 'Default CloudWatch EC2 metrics are hypervisor-level only: CPU, Network, EBS I/O. Memory utilization, swap utilization, and disk-level metrics are NOT collected without the CloudWatch Agent running inside the OS. Enabling detailed monitoring does NOT add these metrics.' },
  { requirement: 'Create multiple AWS accounts with preapproved baselines and governance guardrails', solution: 'AWS Control Tower + Account Factory', why: 'Control Tower automates multi-account setup with blueprints. Account Factory creates new accounts with preapproved network configs and baselines. Guardrails are preventive (SCPs) or detective (Config rules). AWS RAM = resource sharing only. AWS Config = compliance rules only, cannot provision accounts.' },
]
...
const TRAPS: Trap[] = [
  ...
  {
    title: 'Elastic IP cannot attach to ALB — only NLB',
    icon: '🌐',
    trigger: '"Static IP for load balancer" or "clients must whitelist our load balancer IP"',
    wrong: 'Assign an Elastic IP to an Application Load Balancer',
    wrongWhy: 'ALB is DNS-based. Its underlying IP addresses are dynamic and managed by AWS. There is no option to attach an Elastic IP to an ALB.',
    correct: 'Assign Elastic IP to a Network Load Balancer (one per AZ)',
    correctWhy: 'NLB supports one Elastic IP per Availability Zone. This gives clients a fixed IP to whitelist. Optionally put ALB behind NLB if Layer 7 routing is also needed.',
    tip: '"Static IP" + "load balancer" → always NLB. ALB = dynamic DNS-based IPs. If both static IP AND path routing are needed → NLB in front of ALB.',
  },
  {
    title: 'DynamoDB uses a Gateway Endpoint (free) — not an Interface Endpoint',
    icon: '🗄️',
    trigger: '"Private VPC access to DynamoDB without internet"',
    wrong: 'Create an Interface VPC Endpoint for DynamoDB',
    wrongWhy: 'Interface endpoints use AWS PrivateLink and incur per-hour + per-GB charges. DynamoDB does not use PrivateLink and does not have an Interface endpoint.',
    correct: 'Create a DynamoDB Gateway VPC Endpoint (completely free)',
    correctWhy: 'S3 and DynamoDB use Gateway Endpoints — attached to route tables, no hourly or data transfer charges. All other services (Rekognition, Secrets Manager, etc.) use Interface Endpoints.',
    tip: 'Only two services use FREE Gateway Endpoints: S3 and DynamoDB. Every other AWS service uses Interface Endpoints (PrivateLink, costs money). Frequently tested distinction.',
  },
  {
    title: 'RDS Multi-AZ failover is a CNAME flip — not an IP swap or reboot',
    icon: '🔄',
    trigger: '"What happens during an RDS Multi-AZ failover?"',
    wrong: 'AWS reassigns the IP address or creates a new DB instance in the standby AZ',
    wrongWhy: 'IP addresses are subnet-scoped and cannot be moved across AZs. A new instance is not created — the standby already exists. Reboot would not help if the primary AZ is down.',
    correct: 'AWS flips the CNAME record of the DB endpoint to point at the standby (1–2 minutes)',
    correctWhy: 'The standby is promoted to primary and the DB endpoint CNAME is updated. Applications using the endpoint hostname reconnect automatically with no code changes.',
    tip: 'Always use the RDS endpoint DNS name — never hardcode the IP. CNAME flip = ~1–2 min downtime. Multi-AZ standby cannot be read from at any time.',
  },
  {
    title: 'EBS volumes replicate within the same AZ only — not cross-AZ or cross-region',
    icon: '💾',
    trigger: '"EBS provides redundancy across multiple Availability Zones"',
    wrong: 'EBS automatically replicates data across Availability Zones or regions',
    wrongWhy: 'EBS replication is within the same AZ only — protecting against physical hardware failure in that AZ, not against an AZ-level outage.',
    correct: 'EBS snapshots stored in S3 provide cross-AZ durability; snapshots can be copied cross-region',
    correctWhy: 'The EBS volume itself is AZ-locked. Take snapshots (stored in S3, multi-AZ, 11 nines durable) for resilience. Use DLM to automate snapshot lifecycle.',
    tip: 'EBS volume = single AZ (AZ failure = data risk). EBS snapshot = S3 (multi-AZ). You cannot attach an EBS volume to an instance in a different AZ without snapshot → restore.',
  },
  {
    title: 'GuardDuty detects threats — it cannot block anything',
    icon: '🕵️',
    trigger: '"Block suspicious hosts based on GuardDuty security findings"',
    wrong: 'Configure GuardDuty to automatically block malicious IP addresses',
    wrongWhy: 'GuardDuty is a threat detection service. It analyses logs (VPC Flow Logs, CloudTrail, DNS) and generates findings. It takes NO blocking or remediation action on its own.',
    correct: 'GuardDuty findings → EventBridge rule → Lambda → update WAF IP set or NACL deny rule',
    correctWhy: 'Automate response by routing GuardDuty findings to EventBridge, triggering Lambda to add the malicious IP to a WAF IP Set or NACL Deny rule.',
    tip: 'Detection ≠ Prevention. GuardDuty, Inspector, Macie, and Config all detect and alert — none block traffic automatically without a remediation automation.',
  },
  {
    title: 'Auto Scaling cooldown default is 300 seconds — not 600',
    icon: '⏱️',
    trigger: '"Default cooldown period" or "ASG waits before next scaling action"',
    wrong: 'The default Auto Scaling cooldown period is 600 seconds',
    wrongWhy: '600 seconds is a common distractor. There is no 600-second default in any Auto Scaling configuration.',
    correct: 'Default cooldown period is 300 seconds (5 minutes)',
    correctWhy: 'After a scaling activity, ASG waits 300 seconds before evaluating another scaling trigger. Prevents over-scaling in response to transient spikes.',
    tip: 'Cooldown = 300s default. It applies to simple scaling policies only — target tracking and step scaling have their own warm-up settings instead.',
  },
  {
    title: 'S3 server-side encryption uses AES-256 only — not AES-128',
    icon: '🔐',
    trigger: '"Enable SSE-S3 with AES-128" or answers listing AES-128 as an S3 option',
    wrong: 'Enable SSE-S3 using AES-128 encryption',
    wrongWhy: 'AES-128 is not supported by Amazon S3. It is not available as an option in the console, CLI, or API.',
    correct: 'SSE-S3 (and all S3 server-side encryption) uses AES-256 exclusively',
    correctWhy: 'All three S3 SSE options (SSE-S3, SSE-KMS, SSE-C) use 256-bit AES. The request header for SSE-S3 is x-amz-server-side-encryption with value AES256.',
    tip: 'Any answer option mentioning AES-128 for S3 is always wrong. Eliminate immediately. S3 = AES-256 only.',
  },
  {
    title: 'CloudWatch Agent required for memory and swap — detailed monitoring is not enough',
    icon: '📊',
    trigger: '"Monitor EC2 memory utilization" or "track swap space" or "SwapUtilization metric"',
    wrong: 'Enable detailed monitoring on the EC2 instance',
    wrongWhy: 'Detailed monitoring increases metric resolution from 5 minutes to 1 minute — it does NOT add new metric types. Memory and swap are still absent.',
    correct: 'Install the CloudWatch Agent on the EC2 instance',
    correctWhy: 'The CloudWatch Agent runs inside the guest OS and collects OS-level metrics: MemoryUtilization, SwapUtilization, per-disk I/O, per-process CPU. These are invisible to the hypervisor.',
    tip: 'Default CloudWatch EC2 metrics: CPUUtilization, NetworkIn/Out, DiskReadOps/WriteOps (EBS). NOT included: Memory, Swap, filesystem disk usage. Agent required for all OS-level metrics.',
  },
]
...
const CRITICAL_NUMBERS: NumberRow[] = [
  ...
  { service: 'Auto Scaling', fact: 'Default cooldown period', value: '300 seconds (5 minutes)' },
  { service: 'NLB', fact: 'Elastic IP support', value: 'Yes — one EIP per AZ. ALB does NOT support Elastic IPs.' },
  { service: 'ALB', fact: 'Elastic IP support', value: 'None — IPs are dynamic and DNS-based' },
  { service: 'DynamoDB Gateway Endpoint', fact: 'Cost', value: 'FREE — no hourly or per-GB charges (unlike Interface endpoints)' },
  { service: 'S3 Standard-IA', fact: 'Availability SLA', value: '99.9% (vs 99.99% for S3 Standard)' },
  { service: 'S3', fact: 'Server-side encryption algorithm', value: 'AES-256 only — AES-128 is NOT supported' },
  { service: 'EBS', fact: 'Replication scope', value: 'Within the same AZ only — NOT cross-AZ or cross-region' },
  { service: 'EBS Snapshots', fact: 'Storage destination', value: 'Amazon S3 (multi-AZ, 11 nines durability)' },
  { service: 'RDS Multi-AZ', fact: 'Failover mechanism', value: 'CNAME flip from primary endpoint to standby' },
  { service: 'CloudWatch Agent', fact: 'Metrics it adds vs default', value: 'Memory, Swap, disk/filesystem — NOT collected without agent' },
]
