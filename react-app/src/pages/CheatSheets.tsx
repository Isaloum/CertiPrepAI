...
const SHEETS: CertCheatSheet[] = [
  {
    ...
    chooseWhen: [
      ...
      { service: 'FSx for NetApp ONTAP', when: 'Need block storage (iSCSI) shared across both Windows AND Linux instances, or need Multi-AZ file storage', not: 'FSx for Windows (SMB only, no iSCSI) or FSx for Lustre (Linux HPC only, no Windows)' },
      { service: 'Aurora Global Database', when: 'Need cross-region RPO ≤ 1 second and RTO < 1 minute — stricter than RDS can achieve', not: 'RDS cross-region read replicas — those use logical replication with minutes of lag, not storage-level replication' },
      { service: 'AWS Lake Formation', when: 'Share data lake across accounts without copying data, with fine-grained column-level access control on Glue Catalog tables', not: 'S3 cross-account bucket policies — those only control bucket/prefix access, not column-level permissions' },
      { service: 'Egress-only Internet Gateway', when: 'IPv6 instances in a private subnet need outbound internet access without accepting inbound connections', not: 'NAT Gateway — NAT Gateway handles IPv4 only; Egress-only IGW is the IPv6 equivalent for outbound-only traffic' },
      { service: 'AWS Transfer Family', when: 'Partners need to continue using SFTP to deliver files directly into S3 without changing their client configuration', not: 'EC2-based SFTP server — Transfer Family is fully managed with no server maintenance' },
      { service: 'AD Connector', when: 'Company has existing on-premises Active Directory and wants AWS services to authenticate against it without storing any user data in AWS', not: 'Simple AD — Simple AD is standalone (no on-prem AD required). AWS Managed AD — that replicates a full AD into the cloud' },
      { service: 'SAML 2.0 Federation', when: 'Corporate employees need to access the AWS Console or APIs using their existing company credentials via Active Directory / AD FS (single sign-on)', not: 'Web Identity Federation — that is for consumer app users with social logins (Google, Facebook, Amazon)' },
      { service: 'IAM DB Authentication', when: 'Application needs to connect to RDS/Aurora without storing any password — uses EC2 instance role to generate a 15-minute auth token', not: 'Secrets Manager — Secrets Manager still stores a password (just rotates it). IAM DB Auth stores nothing' },
      { service: 'CloudWatch Agent', when: 'You need memory utilization, disk space, or swap metrics from EC2 — these are NOT collected by CloudWatch by default', not: 'CloudWatch detailed monitoring — that only increases frequency of the same default metrics (CPU, network, disk I/O), not memory or disk space' },
      { service: 'RDS Enhanced Monitoring', when: 'You need per-process CPU, memory, or OS-level metrics for an RDS database instance to diagnose which process is consuming resources', not: 'Standard CloudWatch — CloudWatch monitors from the hypervisor (aggregate view only, no per-process breakdown)' },
      { service: 'Aurora Custom Endpoint', when: 'You have multiple Aurora replica sizes in a cluster and want to route specific workloads (analytics, reporting) to a defined subset of larger instances', not: 'Reader endpoint — reader endpoint distributes across ALL replicas automatically; custom endpoint targets a specific subset you define' },
      { service: 'File Gateway', when: 'On-premises apps need NFS/SMB access to files stored in S3, with a local cache for low-latency reads of recently accessed data', not: 'Tape Gateway — Tape Gateway replaces physical tape libraries (no local cache, writes to Glacier)' },
      { service: 'WAF Rate-Based Rule', when: 'You need to automatically block IP addresses that exceed a request rate threshold — no manual IP list, dynamic per-IP enforcement', not: 'Shield Advanced — Shield protects against volumetric DDoS floods (Gbps-level); WAF rate-based rules handle application-layer per-IP throttling' },
      { service: 'S3 Transfer Acceleration + Multipart Upload', when: 'Users around the world need to upload large files (1GB+) to a central S3 bucket as fast as possible', not: 'CRR (Cross-Region Replication) — CRR copies objects between buckets after upload; Transfer Acceleration speeds the upload itself' },
      { service: 'DynamoDB Streams', when: 'You need to react to item-level changes (insert, update, delete) in a DynamoDB table — trigger Lambda, sync to search index, replicate data', not: 'DynamoDB Streams is OFF by default. Must be enabled explicitly. Without enabling, no events fire regardless of trigger configuration' },
      { service: 'AWS RAM (Resource Access Manager)', when: 'You need to share AWS resources (VPC subnets, Transit Gateways, Route 53 Resolver rules) across multiple accounts in your Organization without duplicating infrastructure', not: 'AWS Organizations — Organizations manages accounts and billing. RAM shares specific resources between those accounts. Different jobs' },
    ],
    traps: [
      ...
      { trap: 'Aurora Global vs RDS Cross-Region Read Replicas', why: 'Aurora Global uses storage-level replication: RPO ≤ 1 second, RTO < 1 minute. RDS cross-region replicas use logical replication: lag is minutes, not seconds. When RPO/RTO are aggressive → Aurora Global.' },
      { trap: 'S3 Object Lock: Compliance vs Governance vs Legal Hold', why: 'Compliance mode = nobody (not even root) can delete before the retention period expires. Governance mode = authorized users with s3:BypassGovernanceRetention CAN override. Legal Hold = no time period, indefinite until removed. Compliance is the strictest.' },
      { trap: 'Auto Scaling default termination policy sequence', why: 'Step 1: AZ with the most instances. Step 2: Oldest launch template. Step 3: Instance closest to next billing hour. Step 4: Random. The exam trap is "running longest" — that is NOT in the default policy.' },
      { trap: 'EKS etcd encryption vs EBS encryption', why: 'To encrypt Kubernetes Secrets (stored in etcd), enable secret encryption on the EKS cluster with a KMS key. EBS CSI driver and EBS default encryption encrypt worker node volumes — NOT etcd. These are completely different encryption targets.' },
      { trap: 'S3 single-destination constraint', why: 'S3 event notifications can only deliver to ONE destination per event type (one SNS, one SQS, or one Lambda). To fan out to multiple consumers, route through SNS first — this is the SNS fan-out pattern.' },
      { trap: 'Redis AUTH requires transit encryption', why: '--auth-token (Redis password) only works when --transit-encryption-enabled is also set. You cannot add AUTH after cluster creation. At-rest encryption is separate and does not enable password auth.' },
      { trap: 'Predictive Scaling requires homogenous instances', why: 'Predictive Scaling uses ML to forecast capacity and assumes all instances in the group are the same type and size. Mixed instance types/sizes break its model. Use Scheduled Scaling for mixed fleets instead.' },
      { trap: 'CloudWatch does NOT collect memory or disk by default', why: 'EC2 default metrics: CPU, network, disk I/O — that is it. Memory utilization and disk space are NOT visible to CloudWatch from the hypervisor. Install the CloudWatch Agent inside the OS to push these as custom metrics.' },
      { trap: 'RDS Enhanced Monitoring vs CloudWatch for databases', why: 'CloudWatch monitors RDS from the hypervisor (aggregate view). Enhanced Monitoring runs an agent ON the DB instance and gives per-process CPU, memory, swap, and disk I/O. Choose Enhanced Monitoring when the question mentions per-process or OS-level DB metrics.' },
      { trap: 'IAM DB Authentication uses tokens, not passwords', why: 'IAM DB Auth generates a temporary token (valid 15 minutes) from the EC2 instance role — no password stored anywhere. SSL is automatically enforced. Exam trap: "no password stored" → think IAM DB Auth, not Secrets Manager.' },
      { trap: 'AD Connector vs Simple AD vs AWS Managed AD', why: 'AD Connector = proxy to your existing on-prem AD (no user data in AWS). Simple AD = standalone Samba directory in AWS (no on-prem needed). AWS Managed AD = full Microsoft AD in the cloud. Key: "existing on-prem AD, no data in AWS" → AD Connector.' },
      { trap: 'SAML 2.0 vs Web Identity Federation', why: 'SAML 2.0 = corporate employees using Active Directory/AD FS to access AWS. Web Identity Federation = consumer app users logging in with Google, Facebook, Amazon. "employee SSO" → SAML 2.0. "social login" → Web Identity. Never swap them.' },
      { trap: 'AWS Artifact is not a security scanner', why: 'AWS Artifact = self-service portal to download official compliance documents (SOC 1/2/3, PCI DSS, ISO). Inspector = vulnerability scanning. Security Hub = findings aggregation. Macie = PII in S3. "Auditor needs compliance reports" → Artifact.' },
      { trap: 'Aurora custom endpoints are not automatic', why: 'The reader endpoint balances across ALL read replicas automatically. Custom endpoints target a specific SUBSET — but you must create and configure them explicitly. Exam trap: "route analytics to larger instances" → custom endpoint, not reader endpoint.' },
      { trap: 'Aurora MySQL native function vs RDS event subscriptions', why: 'RDS event subscriptions notify about DB infrastructure events (failover, backup). Aurora MySQL native functions (lambda_sync / lambda_async) invoke Lambda from SQL — triggered by data changes, not infrastructure events.' },
      { trap: 'File Gateway vs Tape Gateway', why: 'File Gateway = NFS/SMB → S3 with local cache for low-latency access. Tape Gateway = replaces physical tape libraries with virtual tapes in Glacier — NO local cache. "Local cache for latency" → File Gateway. "Replace tape backups" → Tape Gateway.' },
      { trap: 'S3 CORS is a browser issue, not a permissions issue', why: 'CORS is enforced by the BROWSER when JavaScript on one domain calls an S3 bucket on another domain. Even if the bucket is publicly readable, the browser blocks it without a CORS policy. Trigger: "browser blocks" or "JavaScript calling another domain" → CORS config on the target bucket.' },
      { trap: 'S3 Transfer Acceleration is for uploads TO S3', why: 'Transfer Acceleration speeds up uploads by routing through CloudFront edge locations. It does NOT serve content to users (that is CloudFront CDN). Combine with Multipart Upload for the fastest large-file upload from distant regions.' },
      { trap: 'Client-side encryption with client master key — nothing sent to AWS', why: 'When encrypting with your own master key client-side, neither the key nor the plaintext ever reaches AWS. AWS receives only ciphertext. "Key never leaves our environment" → client-side with client master key, not SSE-KMS.' },
      { trap: 'Lambda burst scales in seconds — EC2 ASG takes minutes', why: 'Lambda scales to thousands of concurrent executions in SECONDS. EC2 Auto Scaling takes MINUTES to provision and boot. "Respond to sudden spikes instantly" → Lambda. "Minutes" in the answer choice signals EC2 ASG is wrong for burst scenarios.' },
      { trap: 'WAF rate-based rule vs Shield — know the difference', why: 'WAF rate-based rule = auto-blocks IPs exceeding a request rate threshold per 5-minute window. Dynamic, no manual list. Shield Advanced = volumetric layer 3/4 DDoS flood protection. "Block IPs exceeding a rate" → WAF. "Gbps flood attack" → Shield.' },
      { trap: 'Organizations vs RAM — completely different jobs', why: 'AWS Organizations = manage accounts, apply SCPs, consolidated billing. AWS RAM = share specific resources (subnets, Transit Gateways) across accounts. "Share a VPC subnet across accounts" → RAM, not Organizations.' },
      { trap: 'DynamoDB Streams is OFF by default', why: 'DynamoDB Streams must be explicitly enabled on the table. Until enabled, no change records are generated and Lambda triggers produce nothing. Always enable Streams first when building event-driven DynamoDB pipelines.' },
    ],
    architecturePatterns: [
      ...
      { name: 'S3 Fan-Out Pattern', services: ['S3', 'SNS', 'SQS (×N)', 'Lambda'], use: 'S3 can only notify ONE destination. Route to SNS first, then SNS fans out to multiple SQS queues or Lambdas. Classic exam pattern for multi-consumer S3 events.' },
      { name: 'DynamoDB Change Capture', services: ['DynamoDB Streams', 'Lambda', 'SNS / OpenSearch'], use: 'Enable Streams on the table → Lambda reads the stream → push to SNS, update a search index, or replicate cross-region. Streams must be explicitly enabled — off by default.' },
      { name: 'Corporate Identity Federation', services: ['AD FS (on-prem)', 'SAML 2.0', 'IAM Role', 'AWS Console'], use: 'Employee authenticates to on-prem AD via AD FS → SAML assertion → AWS STS AssumeRoleWithSAML → temporary credentials. No IAM users needed. Zero user data stored in AWS.' },
    ],
  },
  {
    id: 'clf-c02',
